    ...
    <c:if test="${not empty categoryStats}">
        ...
        <div class="chart-bars">
            <c:forEach var="stat" items="${categoryStats}" varStatus="vs">
                ...
                <c:if test="${ci == 1}"><c:set var="color" value="#2ecc71" scope="page" /></c:if>
                <c:if test="${ci == 2}"><c:set var="color" value="#e74c3c" scope="page" /></c:if>
                <c:if test="${ci == 3}"><c:set var="color" value="#f39c12" scope="page" /></c:if>
                <c:if test="${ci == 4}"><c:set var="color" value="#9b59b6" scope="page" /></c:if>
                <div class="chart-bar-wrap">
                    <div class="chart-bar-value">${stat.productCount}</div>
                    <div class="chart-bar" style="height: ${stat.productCount * 100 / maxCount}%; background-color: ${color};"></div>
                    <div class="chart-bar-label"><c:out value="${stat.name}" /></div>
                </div>
            </c:forEach>
        </div>
    </c:if>
    <c:if test="${empty categoryStats}">
        <p style="text-align: center; color: #7f8c8d; padding: 40px;">Henüz kategori verisi bulunmamaktadır.</p>
    </c:if>
</div>
